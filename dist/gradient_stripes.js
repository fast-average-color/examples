(function (factory) {
    ...
    function getGradient(image, padding, count) {
        if (padding === void 0) { padding = 30; }
        if (count === void 0) { count = 10; }
        var naturalHeight = image.naturalHeight;
        var height = image.height;
        var naturalHeightPart = Math.floor(naturalHeight / count);
        var heightPart = Math.floor(height / count);
        var parts = [];
        var colors = [];
        ...
        for (var i = 0; i < count; i++) {
            ...
            var top_1 = i ? (i * heightPart) + padding : 0;
            var bottom = ((i + 1) * heightPart - 1) + padding;
            parts.push("".concat(color.rgb, " ").concat(top_1, "px, ").concat(color.rgb, " ").concat(bottom, "px"));
            colors.push(color);
        }
        value += parts.join(', ');
        value += ')';
    ...
    function updateStripes(items) {
        items.forEach(function (item) {
            var image = item.querySelector('img');
            var _a = getGradient(image), value = _a.value, lastColor = _a.lastColor;
    ...
    window.addEventListener('load', function () {
        ...
        window.addEventListener('resize', function () { return updateStripes(items); }, false);
        updateStripes(items);
    }, false);
